...
def flash_attn_benchmark(query,
                         key,
                         value,
                         num_runs=10):
...
if __name__ == "__main__":
    ...
    sdpa_attn_output, sdpa_time = sdpa_benchmark(query,
                                                 key,
                                                 value,
                                                 attn_mask,
                                                 num_runs=num_runs)
    ...
    torch_attn_output, torch_time = torch_benchmark(query,
                                                    key,
                                                    value,
                                                    attn_mask,
                                                    num_runs=num_runs)
    torch.testing.assert_close(sdpa_attn_output,
                               torch_attn_output,
                               atol=1e-3,
                               rtol=1e-3)
    ...
    xformers_attn_output, xformers_time = xformers_benchmark(query,
                                                             key,
                                                             value,
                                                             attn_mask=LowerTriangularMask(),
                                                             num_runs=num_runs)
    torch.testing.assert_close(sdpa_attn_output.transpose(1, 2),
                               xformers_attn_output,
                               atol=1e-3,
                               rtol=1e-3)
    # print("xformers Output:", xformers_attn_output)
    print(f"xformers Time: {xformers_time * 1e3:.6f} ms")

    flash_attn_output, flash_attn_time = flash_attn_benchmark(query,
                                                              key,
                                                              value,
                                                              num_runs=num_runs)
    torch.testing.assert_close(sdpa_attn_output.transpose(1, 2),
